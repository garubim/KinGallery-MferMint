...
export default function MagicMintButton({ isOnGalleryPage = false }: { isOnGalleryPage?: boolean }) {
  ...
  useEffect(() => {
    if (isConnected && address) {
      ...
      // 🔍 DEBUG: Detailed capabilities inspection
      if (availableCapabilities) {
        console.log('🔍 Available Capabilities Detail:', availableCapabilities);
        const baseCapabilities = availableCapabilities[base.id];
        if (baseCapabilities) {
          console.log('🔍 Base Chain Capabilities:', baseCapabilities);
          if (baseCapabilities.paymasterService) {
            console.log('🔍 Paymaster Service:', baseCapabilities.paymasterService);
          }
        }
      }
    }
  }, [isConnected, address, isSmartWallet, capabilities, chain?.id, availableCapabilities]);
